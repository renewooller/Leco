...
public class JmJsUtil {
	...
	private static int resolution_ppqn = Integer.valueOf(System.getProperty(
			"com.lemu.resolution_ppqn", "240"));

	private static double update_resolution_beats = Double.valueOf(System
			.getProperty("com.lemu.update_resolution_beats", "0.25"));

	public final static int StopType = 47; // End of track
	 ...
	 *         phrase tick)
	 */
	private void noteToEvents(Note note, ScoreTracker st)
			throws InvalidMidiDataException {
		int noteOnTick = convertRhythmToPulses(note.getRhythmValue());
		
		if (note.getPitch() != Note.REST) {
			st.currTrack.add(createNoteOnEvent(st.currChannel, note.getPitch(),
					note.getDynamic(), st.phraseTick + noteOnTick));
			long offTick = (long) (st.phraseTick + convertRhythmToPulses(note
					.getDuration()));
			st.currTrack.add(createNoteOffEvent(st.currChannel, note.getPitch(), note.getDynamic(),
					offTick + noteOnTick));
	...
	private void addCallBacksToSeq(double longestTime, Sequence seq) {
		...
		while ((long) (countCallTime * update_resolution_beats * resolution_ppqn) < longestTime) {
			countCallTime++;
			this.addCallback(countCallTime*update_resolution_beats,
							 countCallTime % 64, seq);
		}
	}
	...
	private void addEndEvent(Track longestTrack, double longestTime)
			throws InvalidMidiDataException {
	 ...
	 * @param long tick is the time this event occurs
	 */
	public static MidiEvent createNoteOnEvent(int channel, int pitch,
	 ...
	 * @param long tick is the time this event occurs
	 */
	public static MidiEvent createNoteOffEvent(int channel, int pitch,
			int velocity, long tick) throws InvalidMidiDataException {
