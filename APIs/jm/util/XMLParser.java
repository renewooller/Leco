...
class XMLParser {
    ...
    private static String scoreToXMLString(
        ...
        if (score.getVolume() != Score.DEFAULT_VOLUME) {
            buffer.append(
                    xmlStyle.getSpace()
                    + xmlStyle.getVolumeAttributeName()
                    + "="
                    + xmlStyle.getDoubleQuote()
                    + Integer.toString(score.getVolume())
                    + xmlStyle.getDoubleQuote());
        }
    ...
    private static String partToXMLString(
        ...
        if (part.getVolume() != Part.DEFAULT_VOLUME) {
            buffer.append(
                    ...
                    + "="
                    + xmlStyle.getDoubleQuote()
                    + Integer.toString(part.getVolume())
                    + xmlStyle.getDoubleQuote());
        }
    ...
    private static Score elementToScore(Element element)
        ...
        if (! attributeValue.equals("")) {
            try {
                ...
                throw new ConversionException(
                        "Invalid attribute value: " + attributeValue + ".  The "
                        + "attribute '" + xmlStyle.getVolumeAttributeName() + "' of element '"
                        + xmlStyle.getScoreTagName() + "' must represent a Java double.");
            }
        }
...
//              new ConciseQueryStringXMLStyle() };
    ...
    public static String getVolumeAttributeValue(final Element element) {
        for (int i = 0; i < styles.length; i++) {
            String string =
                    element.getAttribute(styles[i].getVolumeAttributeName());
            if (! string.equals("")) {
                return string;
            }
        }
...
class Element {
    ...
    public String getAttribute(final String string) {
        Enumeration enumr = attributeVector.elements();
        while (enumr.hasMoreElements()) {
            Attribute attribute = (Attribute) enumr.nextElement();
