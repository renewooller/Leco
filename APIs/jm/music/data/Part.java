...
public class Part implements Cloneable, Serializable, JMC{
    ...
    public static final int DEFAULT_CHANNEL = 1; // was 0 17-01-05
	
    public static final int DEFAULT_ID_CHANNEL = -1; // was 0 17-01-05
	...
	*/
    ...
    // an envelope for each of the MIDI controllers
	...
	*/
	...
	public void addCCEnv(int ccType, double [] pos, double [] val) {
		if(pos.length != val.length) {
			try { Exception e = new Exception(" pos and val mus be equal length");
				e.fillInStackTrace();
				throw e;
			} catch(Exception e) {
				e.printStackTrace();
...
//this.getPhraseArray(), 0, this.getSize()-1);
    }
	...
	if(hi0 > lo0) {
	   /* Arbitrarily establishing partition element as the midpoint of
         ...
         while( lo <= hi ) {
             ...
             */
            while( ( lo < hi0 ) && 
		   ( a[lo].getStartTime() < mid.getStartTime() ) )
               ++lo;
             ...
             */
            while( ( hi > lo0 ) && 
		   ( a[hi].getStartTime() > mid.getStartTime() ) )
               --hi;
...
//                    || (ph.getEndTime() > startLoc && ph.getEndTime() <= endLoc)) {
         ...
         * Returns a copy of the Part between specified loactions
         ...
         public Part copy(double startLoc, double endLoc,
                 ...
                 //System.out.println(trimmed + "   " + truncated + "    " +  zeroStartTime + "    " + rests);
                 ...
                 while(enumr.hasMoreElements()){
                         ...
                         // if it starts before the end of the segment to be copied
                         //and if the end of the phrase overlaps into the segment to be copied       
                         
                         if (startTime < endLoc && ph.getEndTime() > startLoc) {
                             
                             Phrase cpy = ph.copy(startLoc, endLoc, trimmed, truncated, zeroStartTime, rests);
                             
                             if(cpy.size() > 1 ||   // if its got nothing in it, don't add it.
                               (cpy.size() == 1 &&  // but if it is rest phrase, and rests are off
                                (rests || !cpy.getNote(0).isRest()))) //don't add it
                                 tempVect.addElement(cpy);  
                         } 
                 }
                 cp.setPhraseList(tempVect);
                 // need to check if it is the right length.. .if it isn't, need to put a rest
                 // in to make the lengh (if rests are required)
                 if(rests) {
                     if(zeroStartTime) { 
                         // check if the endTime is smaller than the required
                         // endtime  
                         if(cp.getEndTime() < ((int)(endLoc*DAC) - (int)(startLoc*DAC))/DAC) {
                             // add a rest to make the diff
        ...
        //         System.out.println("copy -- endtime out " + ((Part)cp).getEndTime());
                 return (Part)cp;
         }
         ...
         public Part copyRT(double st, double en) {
             ...
             int sz = this.size();
             for(int i=0; i< sz; i++) {
                 if(this.getPhrase(i).getStartTime() >= st &&
                         this.getPhrase(i).getStartTime() < en) {
    ...
    * Change both the rhythmValue and duration of each note in the part.
    ...
    public void mergePhrases() {
        // get notes and start times in order
        ...
        for(int i=2; i<coll.length; i++) {
            // shorten if overlapping
            if(n2.getSampleStartTime() < 
	       n1.getSampleStartTime() + n1.getRhythmValue()) {
                n1.setRhythmValue(n2.getSampleStartTime() - n1.getSampleStartTime());
            }
            newPhrase.addNote(n1);
            // add a rest if required
            if(n2.getSampleStartTime() > 
	       n1.getSampleStartTime() + n1.getRhythmValue()) {
                System.out.println("n2 st = " +
				   n2.getSampleStartTime() + 
				   " n1 st = " + n1.getSampleStartTime());
                newPhrase.addNote(new Rest(n2.getSampleStartTime() - 
					   n1.getSampleStartTime() - 
					   n1.getRhythmValue()));
            }
            n1.setSampleStartTime(0); // reset, having used this varibale for sorting
            // get next variable
            n1 = n2;
            n2 = (Note)coll[i];
        }
        ...
        if(n2.getSampleStartTime() < 
	   n1.getSampleStartTime() + n1.getRhythmValue()) {
            n1.setRhythmValue(n2.getSampleStartTime() - 
			      n1.getSampleStartTime());
        }
        newPhrase.addNote(n1);
	...
	public Hashtable copyCCEnv(double from, double to) {
		...
		while(keys.hasMoreElements()) {
			...
			//	PO.p("not Muted " + key.intValue() + " chan = " + this.channel);
				toRet.put(new Integer(key.intValue()), 
						((ValueGraphModel)(ccEnvs.
								get(key))).copySegment(from, to, true));
			} else {
				//PO.p("is muted");
			}
		}
...
// A comparator required by the mergePhrases() becuase the TreeMap implementation 
// did not do what the spec said : (
