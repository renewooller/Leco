...
public final class MidiParser implements JMC{
	 ...
	 * @param boolean cofroze count from zero
	 * @exception Exception
	 */
	//private static double partTempoMultiplier;
//	private static double scoreTempo;
		...
		while(enumr.hasMoreElements()){
			...
			while(partEnum.hasMoreElements()) {
				...
				while(phraseEnum.hasMoreElements()){
					...
					if(note.getPan() != pan) {
						pan = note.getPan();
						addEvent(midiEvents, startTime+offsetValue, 
								new CChange((short)10,(short)(pan * 127), 
					...
					if(pitch != REST) {
						//midiEvents.put(new Double(startTime + offsetValue),
						//		new NoteOn((short)pitch,(short)note.getDynamic(),(short)inst.getChannel(),0));
						addEvent(midiEvents, startTime + offsetValue,
								new NoteOn((short)pitch,
										(short)note.getDynamic(),
										(short)inst.getChannel(),0));
						...
						//midiEvents.put(new Double(endTime + offsetValue),new NoteOn((short)pitch, (short)0, (short)inst.getChannel(),0));
						addEvent(midiEvents, endTime+offsetValue,
								new NoteOn((short)pitch, (short)0, 
										(short)inst.getChannel(),0));
			...
			*/
			...
			for(int index=0;index<keys.length;index++){
				...
				for(int i=1; i< evs.length; i++) {
					evs[i].setTime(0);//-lastime);
					if(evs[i] instanceof NoteOn && ((NoteOn)evs[i]).getVelocity()>0)
						PO.p("    ev " + i + " = " + ((NoteOn)evs[i]).getPitch());
					//lastime = time;
